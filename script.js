...
let cartItems = []; // Move this to global scope
...
function initCart() {
        ...
        if (cartDropdown.classList.contains('visible')) {
            ...
            if (cartItems.length === 0) {
                ...
                const listItems = cartItems.map(item => {
                    ...
                    total += priceValue;
                    return `<li><span>${item.title}</span><strong>${item.price}</strong></li>`;
                }).join('');
    ...
    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('checkout-btn')) {
            ...
            // You can add redirect or modal here
        }
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const modalImg = modal.querySelector(".quick-view-image");
